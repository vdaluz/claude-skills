...
6. Determine target:
   ...
   - **Plane project page**: only if Plane MCP is available AND the user named a project/issue-tracker identifier in the arguments, or confirms one via `AskUserQuestion` when asked which project. Never create against a guessed project. Create via `mcp__plane__create_page` with `project_id` set to the matching project. Title: "PRD: <feature or project name>".
