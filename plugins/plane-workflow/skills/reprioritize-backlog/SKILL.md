...
argument-hint: "[project] [--include-todo] [--limit N]"
...
See `${CLAUDE_PLUGIN_ROOT}/skills/_shared/plane-mcp-gotchas.md` ("`pql`/structured filters can be entirely unsupported") before calling `mcp__plane__list_work_items` - call it with only `project_id`, `expand="state,labels"`, and `fields="id,sequence_id,name,priority,state,labels,target_date,created_at,sort_order,description_html"`. Filter to the Backlog state UUID (and Todo's, if `--include-todo`) yourself over the full result. If the result exceeds the tool's output limit, it's auto-saved to a file - use `jq` on it (`select(.state == "<backlog-uuid>")`) rather than re-requesting with filters. See `${CLAUDE_SKILL_DIR}/reference.md` ("Why description_html is fetched here, and why --limit doesn't shrink this call") for why this fetch is shaped this way.
