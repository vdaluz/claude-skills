...
1. Fetch every issue in the Blocked state.
...
**Don't stop at reporting the blocker table — if your project has a convention for proactively helping with a blocked issue's next unblock step, apply it here, not just when the user separately asks later.** Confirmed once (2026-07-19): a first pass over this step correctly re-checked every Blocked issue and reported a clean blocker table, but stopped there — passively listing what each was blocked on without proposing to walk through the unblock step for the single highest-priority one, even though it was the sole urgent-priority issue in the whole project. Identify the highest-priority issue among the still-blocked set and, for that one specifically, state the concrete next unblock action and offer to walk through it live right now, in the same message as the blocker table — not deferred to a follow-up the user has to ask for. Lower-priority Blocked issues can stay as a plain table entry.
